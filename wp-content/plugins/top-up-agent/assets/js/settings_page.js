...
var setProgressBar = function(id, message, percent) {
        var progressBarValue = jQuery('#' + id + ' .tua-tool-progress-bar-inner');
        var progressBarInfo = jQuery('#' + id + ' .tua-tool-progress-info');
        var progressBarRow = jQuery('#' + id + ' .tua-tool-form-row-progress');
...
var processTool = function ( form, page ) { 
        ...
        var url = security.ajaxurl + '?action=tua_handle_tool_process&_wpnonce=' + security.dropdownSearch;
...
document.addEventListener('DOMContentLoaded', function(event) {
    ...
    jQuery('#tua-generate-tool').submit(function (e){
        e.preventDefault();
        processTool(jQuery(this), 1);
    });
    jQuery('#tua-migrate-tool').submit(function (e){
        e.preventDefault();
        processTool(jQuery(this), 1);
...
(function ($) {
    ...
    'use strict';
    // The "Upload" button
    $(document).on('click', '.tua-field-upload-button', function () {
        window.wpActiveEditor = null;
        var send_attachment_bkp = wp.media.editor.send.attachment;
        var show_attachment_preview = $(this).closest('.tua-field-upload').data('show-attachment-preview')
        var button = $(this);
    ...
    $(document).on('click', '.tua-field-remove-button', function () {
