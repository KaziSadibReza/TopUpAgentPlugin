...
use TopUpAgent\Repositories\Resources\License;
...
class Stock
{
    ...
    {
        add_filter('tua_stock_increase',    array($this, 'increase'),    10, 2);
        add_filter('tua_stock_decrease',    array($this, 'decrease'),    10, 2);
        add_filter('tua_stock_synchronize', array($this, 'synchronize'), 10, 0);
    ...
    private function modify($product, $action, $amount = 1)
    {
        ...
        if (!Settings::get('tua_enable_stock_manager' , Settings::SECTION_WOOCOMMERCE)) {
            return false;
        }
    ...
    public function synchronize() {
        ...
        $args = array(
            ...
            'order'                            => 'ASC',
            'manage_stock'                     => true,
            'tua_licensed_product'           => true,
            'tua_licensed_product_use_stock' => true,
        );
    ...
    {
        if (!empty($query_vars['tua_licensed_product'])) {
            $query['meta_query'][] = array(
                'key'   => 'tua_licensed_product',
                'value' => esc_attr($query_vars['tua_licensed_product'])
            );
        }

        if (!empty($query_vars['tua_licensed_product_use_stock'])) {
            $query['meta_query'][] = array(
                'key'   => 'tua_licensed_product_use_stock',
                'value' => esc_attr($query_vars['tua_licensed_product_use_stock'])
            );
        }
