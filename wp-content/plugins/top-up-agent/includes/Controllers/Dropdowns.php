 ...
 * Class Dropdowns
 *
 * @package TopUpAgent\Controllers
 */
class Dropdowns {
	...
	public function __construct() {
		add_action( 'wp_ajax_tua_dropdown_search', array( $this, 'dropdownSearch' ), 15 );
	}
	...
	public function dropdownSearch() {
		...
		if ( ! current_user_can( 'tua_read_licenses' ) ) {
			wp_die();
		}
		...
		/**
		*	Filter tua_dropdown_searchable_post_types
		*	
		*	@since 1.0
		**/
		$searchable_post_types = apply_filters( 'tua_dropdown_searchable_post_types', array() );
		/**
		*	Filter tua_dropdown_search_query_default_status
		*	
		*	@since 1.0
		**/
		$search_query_status   = apply_filters( 'tua_dropdown_search_query_default_status', array( 'publish' ), $type );

		if ( is_numeric( $term ) ) {
			// Search for a specific license
			if ( 'license' === $type ) {

				/** @var LicenseResourceModel|false $license */
			...
			elseif( 'user' === $type ) {
				...
				/**
				*	Filter tua_dropdown_search_single
				*	
				*	@since 1.0
				**/
				$result = apply_filters( 'tua_dropdown_search_single', [], $type, $term );
				if ( ! empty( $result ) ) {
					$results[] = $result;
				}
			}
            ...
            // }
			 elseif ( 'user' === $type  ) {
				...
				/**
				*	Filter tua_dropdown_search_multiple
				*	
				*	@since 1.0
				**/
				$result = apply_filters(
					'tua_dropdown_search_multiple',
					array(
						'records' => array(),
						'more'    => $more
					),
    ...
    $termHash = apply_filters('tua_hash', $term) . '%';
    $termId = intval($term);
	...
	private function formatProduct( $product ) {
		...
		if ( $product->is_type( 'variable' ) ) {
			$type .= ' (' . __( 'Variable', 'top-up-agent' ) . ')';
		} else if ( $product->is_type( 'variation' ) ) {
			$type .= ' (' . __( 'Variation', 'top-up-agent' ) . ')';
		}
