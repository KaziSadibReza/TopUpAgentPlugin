...
class Export
{
    ...
    {
        add_action('tua_export_license_keys_pdf', array($this, 'exportLicenseKeysPdf'), 10, 1);
        add_action('tua_export_license_keys_csv', array($this, 'exportLicenseKeysCsv'), 10, 1);
        add_action('tua_export_license_keys_by_date_csv', array($this, 'exportLicenseKeysCsv'), 10, 1);
    ...
    {   
        $get_logo = Settings::get( 'tua_company_logo', Settings::SECTION_WOOCOMMERCE );
        ...
        $header = array(
            'id'          => __('ID', 'top-up-agent'),
            'order_id'    => __('Order ID', 'top-up-agent'),
            'product_id'  => __('Product ID', 'top-up-agent'),
    ...
    public function exportLicenseKeysCsv($licenseKeyIds){
        ...
        if (!$columns = Settings::get('tua_csv_export_columns', Settings::SECTION_TOOLS)) {
            $columns = array(
                'id'                  => true,
                'order_id'            => true,
                'product_id'          => true,
            ...
            foreach (array_keys($columns) as $exportColumn) {
                ...
                switch ($exportColumn) {
                    ...
                    case 'status':
                        ...
                        break;
                    default:
                        $getter              = 'get' . tua_camelize($exportColumn);
        ...
        $licenseKeys = apply_filters('tua_export_license_csv', $licenseKeys);
