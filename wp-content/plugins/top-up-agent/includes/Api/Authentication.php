...
class Authentication
{
    ...
    public function authenticate($userId)
    {
        ...
        if (is_ssl() || Settings::get('tua_disable_api_ssl')) {
            ...
            $this->setError(
                new WP_Error(
                    'tua_rest_no_ssl_error',
                    __('The connection is not secure, therefore the API cannot be used.', 'top-up-agent'),
                    array('status' => 403)
                )
            );
    ...
    private function performBasicAuthentication()
    {
        ...
        if (!$consumerKey || !$consumerSecret) {
            $this->setError(
                new WP_Error(
                    'tua_rest_authentication_error',
                    __('Consumer key or secret is missing.', 'top-up-agent'),
                    array('status' => 401)
                )
            );
        ...
        if (empty($this->user)) {
            $this->setError(
                new WP_Error(
                    'tua_rest_authentication_error',
                    __('Consumer key is invalid.', 'top-up-agent'),
                    array('status' => 401)
                )
            );
        ...
        if (!hash_equals($this->user->consumer_secret, $consumerSecret)) {
            $this->setError(
                new WP_Error(
                    'tua_rest_authentication_error',
                    __('Consumer secret is invalid.', 'top-up-agent'),
                    array('status' => 401)
                )
            );
        ...
        switch ($method) {
            ...
            case 'GET':
                if ('read' !== $permissions && 'read_write' !== $permissions) {
                    return new WP_Error(
                        'tua_rest_authentication_error',
                        __('The API key provided does not have read permissions.', 'top-up-agent'),
                        array('status' => 401)
                    );
                }
                break;
            ...
            case 'DELETE':
                if ('write' !== $permissions && 'read_write' !== $permissions) {
                    return new WP_Error(
                        'tua_rest_authentication_error',
                        __('The API key provided does not have write permissions.', 'top-up-agent'),
                        array('status' => 401)
                    );
                }
                break;
            ...
            default:
                return new WP_Error(
                    'tua_rest_authentication_error',
                    __('Unknown request method.', 'top-up-agent'),
                    array('status' => 401)
                );
        }
    ...
    public function sendUnauthorizedHeaders($response)
    {
        if (is_wp_error($this->getError()) && 'basic_auth' === $this->authMethod) {
            $authMessage = __('License Manager for WooCommerce API. Use a consumer key in the username field and a consumer secret in the password field.', 'top-up-agent');
    ...
    public function checkUserPermissions($result, $server, $request)
    {
        ...
        $error = apply_filters('tua_rest_api_validation', $result, $server, $request);
