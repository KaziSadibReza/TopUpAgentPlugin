<?php
/**
 * Top Up Agent - License functions
 *
 ...
 * @throws Exception
 */
function tua_add_license($licenseKey, $licenseData = array())
{
    ...
    $encryptedLicenseKey = apply_filters('tua_encrypt', $licenseKey);
    $hashedLicenseKey    = apply_filters('tua_hash', $licenseKey);
 ...
 * @throws Exception
 */
function tua_get_license($licenseKey)
{
    /** @var LicenseResourceModel $license */
 ...
 * @return bool|LicenseResourceModel[]
 */
function tua_get_licenses($query)
{
    if (array_key_exists('license_key', $query)) {
        $query['hash'] = apply_filters('tua_hash', $query['license_key']);
 ...
 * @throws Exception
 */
function tua_update_license($licenseKey, $licenseData)
{
    $updateData = array();
    ...
    if (array_key_exists('license_key', $licenseData)) {
        ...
        if (apply_filters('tua_duplicate', $licenseData['license_key'], $oldLicense->getId())) {
            throw new Exception(esc_html("The license key '{$licenseData['license_key']}' already exists."));
        }

        $updateData['license_key'] = apply_filters('tua_encrypt', $licenseData['license_key']);
        $updateData['hash']        = apply_filters('tua_hash', $licenseData['license_key']);
    }
    ...
    $newLicenseHash = apply_filters('tua_hash', $licenseKey);
    ...
    if ($newLicense->getProductId() !== null && $newLicense->getStatus() === LicenseStatusEnum::ACTIVE) {
        apply_filters('tua_stock_increase', $newLicense->getProductId());
    }
 ...
 * @throws Exception
 */
function tua_delete_license($licenseKey)
{
    /** @var LicenseResourceModel $oldLicense */
    $oldLicense = LicenseResourceRepository::instance()->findBy(
        array(
 ...
 * @throws Exception
 */
function tua_activate_license($licenseKey, $args)
{
   ...
   try {
    ...
    if ( isset($args['token']) && !empty($args['token']) ) {
        $licenseData = tua_reactivate_license($args['token']);
    ...
    try {
        ...
        $newToken = tua_generateToken( $licenseKey );
        if ( is_null( $newToken ) ) {
        ...
        $activationParams = array(
            ...
            'ip_address' => tua_clientIp(),
            'user_agent' => tua_userAgent()
        );
        ...
        if ( ! $licenseActivation ) {
            return new WP_Error( 'server_error', __( 'Unable to activate key', 'top-up-agent' ), array( 'status' => 500 ) );
        }
    ...
    $licenseData = apply_filters( 'tua_activate_license_data', $licenseData );
     ...
     * @throws Exception
     */
     function tua_deactivate_license($licenseKey, $args)
    {
        if ( isset($args['token']) && !empty($args['token']) ) :
            try {
                /** @var \TopUpAgent\Models\Resources\LicenseActivation $activation */
    ...
    function tua_delete_activation($activation_id, $license_id)
    {
 ...
 /**
     ...
     * @throws Exception
     */
 function tua_reactivate_license($activation_token)
 {
     ...
     * @return string|null
     */
    function tua_generateToken( $licenseKey ) {
        $token    = apply_filters('tua_activation_hash',  $licenseKey );
        $reps     = 0;
        $max_reps = 20;
        while ( true ) {
